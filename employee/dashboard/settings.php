...
if(isset($_SESSION['dbu'])){
  $sql = "SELECT id,firstname,middlename,lastname,gender,phone,email,username FROM tbl_employee WHERE id=?";
  $qry = $connection->prepare($sql);
  $qry->bind_param("i",$_SESSION['dbu']);
  $qry->execute();
  $qry->bind_result($id,$dbf,$dbm,$dbl,$dbg,$dbp,$dbe,$dbuser);
  $qry->store_result();
  $qry->fetch ();
}
          ...
          <div class="box">
            ...
            <div class="box-body">
              ...
              <div class="col-md-6">
                ...
                <input type="text" class="form-control" name="email" value="<?php echo $dbe; ?>"  required>
...
if(isset($_POST['btnSave'])){
    ...
    if($_POST['password'] != ''){
      ...
      $sql = "UPDATE tbl_employee SET firstname=?,middlename=?,lastname=?,gender=?,phone=?,email=?,password=?,username=? WHERE id=?";
      $qry = $connection->prepare($sql);
      $qry->bind_param("ssssssssi",$_POST['firstname'],$_POST['middlename'],$_POST['lastname'],$_POST['gender'],$_POST['phone'],$_POST['email'],$hashed_password,$_POST['username'],$_SESSION['dbu']);
    }else{
      $sql = "UPDATE tbl_employee SET firstname=?,middlename=?,lastname=?,gender=?,phone=?,email=?,username=? WHERE id=?";
      $qry = $connection->prepare($sql);
      $qry->bind_param("sssssssi",$_POST['firstname'],$_POST['middlename'],$_POST['lastname'],$_POST['gender'],$_POST['phone'],$_POST['email'],$_POST['username'],$_SESSION['dbu']);
    }
